...
const Dashboard = () => {
  ...
  const getCategoryBadge = (category) => {
    ...
    return (
      <span className={`bg-${categoryInfo.color.replace('bg-', '')} text-white inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium`}>
        {categoryInfo.name}
      </span>
    );
  };
  ...
  return (
    <div className="max-w-7xl mx-auto">
      ...
      {issues.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {issues.map((issue) => (
            <div key={issue._id} className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 hover:shadow-md transition-shadow duration-200 cursor-pointer">
              <div className="mb-3">
