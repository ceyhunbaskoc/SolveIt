# SolveIt

---

## Proje Hakkında

![Ürün Tanıtım Görseli](docs/Product.png)

**Proje Tanımı:** > SolveIt, Süleyman Demirel Üniversitesi kampüs sınırları içerisinde ve Isparta şehir genelinde karşılaşılan teknik arızalar, çevre kirliliği veya altyapı sorunlarının yetkililere hızlıca iletilmesini sağlayan bütünleşik bir sorun bildirim platformudur. Öğrenciler ve vatandaşlar, karşılaştıkları problemleri tam konum ve kategori bilgisiyle birlikte sisteme kolayca kaydedebilirler. Geleneksel dilekçe veya mail yöntemlerinin aksine SolveIt, kullanıcılara tıpkı yemek siparişi uygulamalarındaki gibi anlık durum takibi ("Beklemede", "İnceleniyor", "Çözüldü") imkanı sunar. Platformun amacı, yetkililerin iş yükünü önem sırasına göre organize etmesini kolaylaştırırken, kullanıcıların da kendi bildirdikleri sorunların çözüldüğünden şeffaf bir şekilde emin olmalarını sağlamaktır.

**Proje Kategorisi:** > Şehir ve Kampüs Yönetimi / Akıllı Şehir Uygulamaları

**Referans Uygulama:** > Yemeksepeti / Getir (Anlık statü takibi mimarisi referans alınmıştır) ve FixMyStreet (Sorun bildirim konsepti)

---

## Proje Linkleri

- **REST API Adresi:** `http://localhost:5000/api` *(Geliştirme Ortamı)*
- **Web Frontend Adresi:** `http://localhost:3000` *(Geliştirme Ortamı)*

---

## Proje Ekibi

**Grup Adı:** > SingleThread

**Ekip Üyeleri:** - Ceyhun Başkoç

---

## Dokümantasyon

Proje dokümantasyonuna aşağıdaki linklerden erişebilirsiniz. *(Solo geliştirici olunduğu için tüm gereksinimler tek üye üzerinde toplanmıştır).*

1. [Gereksinim Analizi](docs/Gereksinim-Analizi.md)
2. [REST API Tasarımı](docs/Rest-API.md)
3. [REST API Görevleri](docs/Ceyhun-Baskoc-Rest-API-Gorevleri.md)
4. [Web Front-End](docs/WebFrontEnd.md)
5. [Mobil Front-End](docs/MobilFrontEnd.md)
6. [Mobil Backend](docs/MobilBackEnd.md)
7. [Video Sunum](docs/Sunum.md)

---